...
BYTE InValue(BYTE t) {
  ...
  LATB=t;   // SOLO BYTE BASSO??
  TRISE=0b11111111;
  mMREQ=1; mIOREQ=0; mWR=1; mRD=0; 
  DELAY_IO_RD();
#ifdef __PIC32
  return PORTE;
#else
...
//  Pipe1=GetValue(t++);
//  Pipe2.bytes.byte1=GetValue(t++);
//  Pipe2.bytes.byte2=GetValue(t);
  ...
  DELAY_MEM_RD();
  Pipe1=PORTE;
  // VERIFICARE se qua ha senso! potrebbe dare problemi...
  // FARE MACRO GetValue e usarla man mano nelle varie istruzioni...
  LATB++;
  DELAY_MEM_RD();
#ifdef __PIC32
  Pipe2.bb[0] = PORTE;
...
// printf("rom_seg: %04x, p: %04x\n",rom_seg,p);
  ...
  LATB=t;
  mIOREQ=1; mMREQ=0; mRD=mWR=1;
  TRISE=0b00000000;
  DELAY_MEM_WR();
#ifdef __PIC32
  LATE=t1;
#else
  LATC=t1; OCCHIO altri bit qua!
#endif
  mWR=0;  
  }
...
// printf("rom_seg: %04x, p: %04x\n",rom_seg,p);
  ...
  LATE=LOWORD(t1);
#else
  LATC=LOWORD(t1); OCCHIO altri bit qua!
#endif
  LATB++;
  DELAY_MEM_WR();
#ifdef __PIC32
  LATE=HIWORD(t1);
#else
  LATC=HIWORD(t1); OCCHIO altri bit qua!
#endif
  mWR=0;  
  }
...
//			printf("%04x    %02x\n",_pc,GetValue(_pc));
      ...
      _i=_r=0;
		...
		if(!mIRQ) {
#warning FARE SOLO SU FRONTE DISCESA?! anche NMI??
			if(IRQ_Enable1) {
...
//        keysFeedPtr=0;
      ...
      LED1^=1;    // ~700nS 7/6/20, ~600 con 32bit 10/7/21 MA NON FUNZIONA/visualizza!! verificare
